...
export default function Panel({ title, tone = C.amber, bg = C.green, children, style, bodyStyle, titleRight }) {
  return (
    <div style={{
      position: "relative", border: `3px solid ${tone}`, borderRadius: 8,
      padding: title ? "16px 10px 10px" : 10, marginTop: 18, background: bg,
      ...style,
    }}>
      {title && (
        <div style={{
          position: "absolute", top: 0, left: "50%", transform: "translate(-50%, -55%)",
          background: bg, padding: "0 8px", fontFamily: PIXEL, fontSize: 10, zIndex: 1,
